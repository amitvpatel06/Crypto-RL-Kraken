...
class Q:
	...
	def generate_state_vec(self, state):
		vec = []
		for key in sorted(state.keys()):
			vec.append(state[key])
		return ','.join([str(x) for x in vec])

	def update_table(self, key_dict, action, reward):
		# create keys to index into table
		num_key = str(action)+ ',n'
		key = self.generate_state_vec(key_dict)

		if self.backup['name'] == "sampling":
			# set values appropriately for new keys
			n = self.update_table_keys(key, num_key, action, self.Q)
			self.Q[key][action] = float(n)/(n+1)*self.Q[key][action] + float(1)/(n+1)*(reward)
			...
			use_Q[key][action] = float(n) / (n+1) * use_Q[key][action] + float(1) / (n+1)*(reward)
			use_Q[key][num_key] += 1
	...
	def arg_min(self, key_dict, table=None):
		key = self.generate_state_vec(key_dict)
		if table is None:
			table = self.Q
		min_action = -1
