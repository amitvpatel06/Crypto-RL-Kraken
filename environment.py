...
class Environment:
	...
	def get_timesteps(self, start, end, I, V, window=10):
		...
		for i in range(start, end):
			...
			if len(self.time_steps) > 0:
				ob, n_v, _ = curr_book.diff(ask_prices, ask_volumes, bid_prices, bid_volumes)
				ob.set_s_vol(n_v)
	...
	def volume(self, curr_book):
		total = 0
		a_volumes = list(curr_book.a.values())
		b_volumes = list(curr_book.b.values())
	...
	# returns average price paid
	def vol_order(self, curr_book, side, volume):
		# 0 is buy, 1 is sell
		total = 0
		shares = volume
		if side == 0:
...
class OrderBook:
	...
	def get_spread(self):
		asks = list(self.a.keys())
		bids = list(self.b.keys())
		return min(asks) - max(bids)

	def get_misbalance(self):
		total = 0
		a_volumes = list(self.a.values())
		b_volumes = list(self.b.values())
		for i in range(len(a_volumes)):
			total += a_volumes[i] - b_volumes[i]
		return total
	...
	def ob_diff(self, ob):
		asks = list(ob.a.keys())
		ask_vols = list(ob.a.values())
		bids = list(ob.b.keys())
		bid_vols = list(ob.b.values())
		ob, nv, tv = self.diff(asks, ask_vols, bids, bid_vols)
		return ob, nv, tv 
	...
	def diff(self, asks, ask_vols, bids, bid_vols):
		...
		for i in range(len(asks)):
			if asks[i] in self.a:
				...
				total_vol += ask_vols[i] - self.a[asks[i]]
				net_vol	+= ask_vols[i] - self.a[asks[i]]
			else:
				new_a.append(asks[i])
				new_av.append(ask_vols[i])
				total_vol += ask_vols[i] 
			if bids[i] in self.b:
				new_b.append(bids[i])
				new_bv.append(bid_vols[i] - self.b[bids[i]])
				total_vol += bid_vols[i] - self.b[bids[i]]
				net_vol	-= bid_vols[i] - self.b[bids[i]]
			else:
				new_b.append(bids[i])
				new_bv.append(bid_vols[i])
				total_vol += bid_vols[i]
		return OrderBook(new_a, new_av, new_b, new_bv), net_vol, total_vol
	...
	is no longer available to order in our books! All we defo when we move timesteps
	...
	def immediate_value(self, side, units):
		if side == 0:
			return self.immediate_cost_buy(units)
		else:
			return self.immediate_cost_sell(units)

	
	def immediate_cost_sell(self, units):
		total = 0
		rem = units
		for pr, v in sorted(self.b.items())[::-1]:
			if rem > v:
				total += pr * v
				rem -= v
			else:
	...
	def vectorize_book(self, price_levels, time, inv):
		...
		row = 0
		a_prices = list(self.a.keys())
		a_prices.sort(key=int)
		b_prices = list(self.b.keys())
